...
- [x] **[Eng] One-click "Create client workspace".** ✅ `npm run client:create -- --name "…" --email owner@…` creates the tenant + pending Owner and sends the Clerk invite in one command (no allowlist/manual step). _(NH-102.)_
...
- [x] **[Biz/Eng] Error tracking + uptime monitoring.** ✅ Live: Sentry (EU region) receiving errors — confirmed end-to-end via a smoke test; `/api/health` DB probe public + monitored by UptimeRobot (5-min checks).
