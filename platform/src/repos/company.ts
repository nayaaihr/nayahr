...
export type Company = { name: string; logoUrl: string | null; nameConfirmed: boolean };
...
export async function setCompanyName(s: Session, name: string): Promise<void> {
  ...
  await withSession(s, async (tx) => {
    // Setting the name also marks it confirmed, clearing the onboarding prompt.
    await tx.execute(sql`update tenant set name = ${clean}, name_confirmed = true where id = ${s.tenantId}`);
